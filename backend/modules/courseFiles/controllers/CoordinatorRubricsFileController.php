...
class CoordinatorRubricsFileController extends Controller
{
    ...
    public function actionPage($id)
    {
		...
		if ($model->load(Yii::$app->request->post())) {
			...
			if(Yii::$app->request->post('complete') == 1){
				$model->progressContRubrics = 1;
			}else{
				$model->progressContRubrics = 0;
			}
			if(Yii::$app->request->post('na') == 1){
				$model->na_cont_rubrics = 1;
				$model->progressContRubrics = 1;
			}else{
				$model->na_cont_rubrics = 0;
			}
			//echo $model->progressContRubrics ;die();
            ...
            if($valid){
				...
				try {
					...
					if($progress and $model->prg_cont_rubrics == 0){
						$model->progressContRubrics = 0.5;
						$model->save();
					}
    ...
    public function actionDeleteRow($id){
		...
		$model->offered->na_cont_rubrics = 0;
		$model->offered->progressContRubrics = 0.5;
		$model->offered->save();
