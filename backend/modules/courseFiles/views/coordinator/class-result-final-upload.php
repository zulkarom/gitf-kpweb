...
$course = $model->course;
$title = 'Final Exam or Assessment Results';
$this->title = $course->course_code . ' ' . $course->course_name;
$this->params['breadcrumbs'][] = ['label' => 'My Course File', 'url' => ['/course-files/default/teaching-assignmen']];
...
<tbody>
	...
	if($files){
		...
		foreach($files as $x=>$file){
			...
			$file->file_controller = 'coordinator-result-final-file';
			?>
			<tr>
			<td><?=$i?>. </td>
			<td>
...
<?php 
$check_na = $model->na_result_final == 1 ? 'checked' : ''; 
$check_complete = $model->prg_result_final == 1 ? 'checked' : ''; 
?>
