...
use backend\modules\postgrad\models\StudentData5;
 ...
 */
class StudentImport2Controller extends Controller
{
    /**
     * {@inheritdoc}
     */
    ...
    public function actionImport()
    {
        ...
        $srcRows = StudentData5::find()->where(['DONE' => 0])->limit(50)->all();
        foreach ($srcRows as $stud) {
    ...
    private function mapNric($src) { return str_replace('-', '', (string)$src->NO_IC_PASSPORT); }
    ...
    private function mapGender($src) { return $src->gender; }
    
    private function mapNationality($src)
    {
        return $src->NEGARA_ASAL_KOD;
    }
    private function mapCitizenship($src) { return $src->citizenship; }
...
private function mapRace($src)
{
    ...
    private function mapCurrentSemester($src) { return (string)($src->SEMESTER_SEMASA_PELAJAR?? ''); }
...
private function mapStudyOfferCondition($src){
    return trim((string)$src->JENIS_TAWARAN);
}
