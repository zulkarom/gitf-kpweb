...
class Publication extends \yii\db\ActiveRecord
{
    ...
    {
        return $this->hasOne(PubType::className(), [
            'id' => 'pub_type'
        ]);
    }
    ...
    {
        return $this->hasMany(Editor::className(), [
            'pub_id' => 'id'
        ]);
    }
    ...
    {
        return $this->hasMany(PubTag::className(), [
            'pub_id' => 'id'
        ]);
    }
    ...
    public function getMyPubTags()
    {
        return $this->hasMany(PubTag::className(), [
            ...
            'rp_pub_tag.staff_id' => Yii::$app->user->identity->staff->id
        ]);
    }
    ...
    public function getPubTagsNotMe()
    {
        return $this->hasMany(PubTag::className(), [
            ...
            '<>',
            'staff_id',
            Yii::$app->user->identity->staff->id
        ]);
    }

    public function tagStaffArray()
    {
        $list = self::find()->select('staff.id, user.fullname as staff_name, user.id as user_id')
    ...
    {
        $years = self::find()->select('DISTINCT(pub_year)')
    ...
    {
        $years = self::find()->select('DISTINCT(pub_year)')->all();
        $array = [];
        if ($years) {
            $array = ArrayHelper::map($years, 'pub_year', 'pub_year');
        }
        $cur = date('Y');
